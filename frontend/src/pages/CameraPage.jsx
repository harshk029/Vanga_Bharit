...
export default function CameraPage({ feedUrl, title, subtitle, color, hideAlerts = false, showReset = false, enableSiren = false, enableTrespassSiren = false, trespassOnly = false, weaponsOnly = false }) {
  ...
  const playWeaponAlarm = () => {
    if (!enableSiren) {
      console.log("[Audio] Gate BLOCKED siren for this camera.");
      return; 
    }
    console.log("[Audio] Gate ALLOWED siren START.");
    try {
  ...
  const playTrespassAlarm = () => {
    if (!enableTrespassSiren) return;
    try {
      const ctx = new (window.AudioContext || window.webkitAudioContext)();
      const masterGain = ctx.createGain();
      ...
      // Piercing high-low siren loop
      osc.type = 'square';
      osc.frequency.setValueAtTime(900, ctx.currentTime);
      osc.frequency.setValueAtTime(1300, ctx.currentTime + 0.3);
      osc.frequency.setValueAtTime(900, ctx.currentTime + 0.6);
      osc.frequency.setValueAtTime(1300, ctx.currentTime + 0.9);
      ...
      console.warn('Trespass Audio failed:', e);
    }
  };
  ...
  useEffect(() => {
    ...
    const unsubM = wsService.onMessage((d) => {
      if (d?.level === 'alert') {
        ...
        if (enableTrespassSiren && d.msg?.includes('TRESPASS')) {
          playTrespassAlarm();
        }
        // If trespassOnly, ignore anything that isn't a trespass alert
        if (trespassOnly && !d.msg?.includes('TRESPASS')) return;
        // If weaponsOnly, ignore anything that isn't a weapon alert
        if (weaponsOnly && !d.msg?.includes('WEAPON DETECTED')) return;
